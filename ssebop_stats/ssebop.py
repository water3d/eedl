...
def _get_fiona_args(polygon_path):
	...
	parts = polygon_path.split()
	if parts[0].endswith(".gdb") and "." not in parts[1]:  # if the folder name ends with .gdb and the "filename" doesn't have an extension, assume it's an FGDB
...
class SSEBOPer(object):
	...
	def zonal_stats(self, polygons, keep_fields=("UniqueID", "CLASS2"), stats=('min', 'max', 'mean', 'median', 'std', 'count', 'percentile_1', 'percentile_5', 'percentile_10', 'percentile_90', 'percentile_95', 'percentile_99')):
		...
		zstats_results_geo = rasterstats.gen_zonal_stats(fiona.open(**_get_fiona_args(polygons)), self.mosaic_image, stats=stats, geojson_out=True, nodata="")
		...
		with open(os.path.join(self.output_folder, f"{self.filename}_zstats.csv"), 'w') as csv_file:
